...
describe('ExactCalldata', () => {
  ...
  describe('decodeExactCalldataTerms', () => {
    ...
    it('decodes terms created from Uint8Array calldata', () => {
      const callDataBytes = new Uint8Array([
        0x12, 0x34, 0x56, 0x78, 0x90, 0xab, 0xcd, 0xef,
      ]);
      expect(
    ...
    it('accepts Uint8Array terms from the encoder', () => {
      const calldata = '0x1234567890abcdef' as Hex;
      const bytes = createExactCalldataTerms({ calldata }, { out: 'bytes' });
      expect(decodeExactCalldataTerms(bytes)).toStrictEqual({ calldata });
    });
  });
});
