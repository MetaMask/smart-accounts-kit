...
describe('ExactCalldataBatch', () => {
  ...
  describe('decodeExactCalldataBatchTerms', () => {
    const targetA: `0x${string}` = '0x0000000000000000000000000000000000000003';
    const targetB: `0x${string}` = '0x0000000000000000000000000000000000000004';

    const executions = [
      { target: targetA, value: 0n, callData: '0xdeadbeef' as `0x${string}` },
      { target: targetB, value: 5n, callData: '0x' as `0x${string}` },
    ];

    it('decodes multiple executions', () => {
      const original = { executions };
      expect(
        decodeExactCalldataBatchTerms(createExactCalldataBatchTerms(original)),
      ).toStrictEqual(original);
    });

    it('accepts Uint8Array terms from the encoder', () => {
      const original = { executions };
      const bytes = createExactCalldataBatchTerms(original, { out: 'bytes' });
      expect(decodeExactCalldataBatchTerms(bytes)).toStrictEqual(original);
    });
  });
});
