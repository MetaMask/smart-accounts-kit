...
describe('ExactExecutionBatch', () => {
  ...
  describe('decodeExactExecutionBatchTerms', () => {
    const targetA: `0x${string}` = '0x0000000000000000000000000000000000000001';
    const targetB: `0x${string}` = '0x0000000000000000000000000000000000000002';

    const executions = [
      { target: targetA, value: 1n, callData: '0x1234' as `0x${string}` },
      { target: targetB, value: 2n, callData: '0x' as `0x${string}` },
    ];
    ...
    it('accepts Uint8Array terms from the encoder', () => {
      const original = { executions };
      const bytes = createExactExecutionBatchTerms(original, { out: 'bytes' });
      expect(decodeExactExecutionBatchTerms(bytes)).toStrictEqual(original);
    });
  });
});
